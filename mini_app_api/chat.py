...
- "case_status" — питання про стан ЙОГО справи (етап, оплати, документи, наступні кроки). Відповідай на основі даних про справу.
- "faq" — загальне питання про банкрутство/борги/колекторів/арбітражного керуючого/суд, яке покривається базою знань нижче. Відповідай виключно на основі бази знань.
...
- "uncertain" — питання по темі, але в базі знань немає впевненої відповіді. Чесно скажи, що уточниш.
...
def classify_and_reply(
    *, case_summary: str, faq_matches: list[dict], history: list[dict], user_message: str, prior_summary: str | None = None,
    ...
    if category == "off_topic" or not reply:
        ...
        reply = OFF_TOPIC_REPLY if category == "off_topic" else "Дайте мені трохи часу — уточню це і повернуся з відповіддю."
